...
function openSnippetsPage(version, reason){
	...
	if(version === "3.0.0.1" && reason === "update")
		localStorage.prepare300Update = true;
}

// create modal dialog for blocking site by detector.js
(function createBlockSiteModal(){
	var modalContent = "<div class='prokeys-block prokeys-block-theme-plain'>\
		<div class='prokeys-block-overlay'></div>\
		<div class='prokeys-block-content'>\
			<div class='prokeys-block-dialog-form'>\
				<div class='prokeys-block-dialog-message'>Are you sure you want to <span class='action'></span><br> <input type='text' class='site-name'><br> from ProKeys?</div>\
				<div class='prokeys-block-dialog-buttons'>\
					<input type='button' value='OK' class='prokeys-block-dialog-button-primary prokeys-block-dialog-button'>\
					<input type='button' value='Cancel' class='prokeys-block-dialog-button-secondary prokeys-block-dialog-button'> </div>\
			</div>\
		</div></div>";
		...
		if(needToGetLatestData){
			chrome.tabs.sendMessage(tabs[0].id, {giveSnippetList: true}, function(response){
				if(Array.isArray(response)){
					...
					Data.snippets = Folder.fromArray(response);
				}
			});		
		}
	});
...
function addCtxSnippetList(snippets, addMainEntryFlag){
	...
	console.dir(Data.snippets); console.dir(hasSnippets);
