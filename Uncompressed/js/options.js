...
(function(){
	...
	var storage = chrome.storage.local,
		...
		SHOW_CLASS = "shown",
		$tabKeyInput,
		$snipMatchWholeWordInput, $snipNameDelimiterListDIV,
		ORG_DELIMITER_LIST = " ,.+=!@#%^&*()[]{}\\/\"'|<>~`";		
	...
	function appendInputBoxesInTable(){
		...
		append(document.createElement("button").html("Save"));
		...
		validateRestoreData = function(data, snippets){
			...
			var propCount = 0,
				...
				correctProps = ["blockedSites", "charsToAutoInsertUserList", "dataVersion",
						...
						"matchWholeWord", "snipNameDelimiterList"],
				msg = "Data had invalid property: ";

			// ensure backwards compatibility
			data.dataUpdateVariable = false; // #backwardscompatibility
			setPropIfUndefined("matchWholeWord");// #backwardscompatibility
	...
	function editPanel(type){
		...
		return function(object, isClosingEditPanel){
			var	headerSpan = $panel.querySelector(".header span"),
				...
				bodyElm = $panel.querySelector(".body-editor"),				
			...
			if(isEditing){
				...
				if(bodyElm) bodyElm.html(object.body);			
			}
			else{
				nameElm.text("").focus();
				if(bodyElm) bodyElm.html("");
	...
	function commonValidation(panelName){
		...
		function manipulateElmForValidation(elm, validateFunc){
			var text = Snip.removeSpamDivNodes(elm),
	...
	function init(){		
		...
		$snipMatchWholeWordInput = $(".snippet_match_whole_word input[type=checkbox]");
		$tabKeyInput = $("#tabKey");
		...
		(function settingsPageHandlers(){
			...
			$snipMatchWholeWordInput.on("change", function(){
				var isChecked = this.checked;
				Data.matchWholeWord = isChecked;
				$snipNameDelimiterListDIV.toggleClass(SHOW_CLASS);
				saveOtherData("Data saved!");
			});

			if(!Data.snipNameDelimiterList){
				Data.snipNameDelimiterList = ORG_DELIMITER_LIST;
				setDelimiterListRegex();
				// so that Data.snippets being changed to array 
				// doesn't affect any thing
				setTimeout(saveOtherData, 500);
			}
	...
	function setEssentialItemsOnDBLoad(){
		// somehow the update broke everything so I have to set it in right order
		...
		$snipMatchWholeWordInput.checked = Data.matchWholeWord;
		
		if(Data.matchWholeWord)
